#!/usr/bin/env python
"""Task runner for the python-boilerplate project."""
...
def autopep8():
    """Autopep8 modules."""
    shell('autopep8 -i -r fabfile.py python_boilerplate/ tests/')
...
def coverage(no_browser):
    ...
    shell('coverage run --source python_boilerplate -m pytest')
...
def docs(no_browser):
    ...
    """
    shell('rm -f docs/python_boilerplate.rst')
    shell('rm -f docs/modules.rst')
    shell('rm -f docs/python_boilerplate*')
    shell('sphinx-apidoc -o docs/ python_boilerplate')
